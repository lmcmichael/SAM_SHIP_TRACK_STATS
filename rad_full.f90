...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	USE params, ONLY: &
		...
		zenith_angle, & ! zenith angle if doperpetual==dosolarconstant==.true.
                ocean_flux_option ! If cool skin option is used, use tskinxy as surface temperature
	...
	USE vars, ONLY: &
		...
		sstxy, t00, tskinxy, & ! SST perturbation and 300 K, Skin temperature
	...
	IF ( icycle == 1 ) THEN ! Skip subcycles (i.e., when icycle /= 1)
		...
		IF ( .NOT.isInitialized_RadDriver ) THEN
			...
			ENDIF
                        ...
                        if(doShipTrackConditionals.and.(.not.isAllocatedRadStreams)) then
                          !allocate 3-D radiation stream arrays
                          allocate(swUp3D(nx,ny,nzm), swDown3D(nx,ny,nzm), lwUp3D(nx,ny,nzm), lwDown3D(nx,ny,nzm), STAT=ierr)
                          if(ierr.ne.0) then
                            write(*,*) 'Cannot allocate individual radiation streams arrays in initialize_radiation'
                            call task_abort()
                          end if
                          isAllocatedRadStreams = .true.
                        end if
...
!bloss(2018-02)                          rad_reffc(1:nx,1:ny,1:nzm) = reffl(1:nx,1:ny,1:nzm)
			...
			DO j = 1, ny
				...
				tg_slice(1:nx) = sstxy(1:nx,j) + t00
                                IF(ocean_flux_option.eq.36) THEN
                                  tg_slice(1:nx) = tskinxy(1:nx,j)
                                END IF
				...
				qrad(1:nx,j,1:nzm) = swHeatingRate(1:nx,1:nzm) + lwHeatingRate(1:nx,1:nzm)
                                ...
                                ! Save heating rates for MSE budget output
                                if(do_chunked_energy_budgets) then
                                  qrad_lw(:, j, :) = lwHeatingRate(:, 1:nzm)
                                  qradclr_lw(:, j, :) = lwHeatingRateClearSky(:, 1:nzm)
                                  qrad_sw(:, j, :) = swHeatingRate(:, 1:nzm)
                                  qradclr_sw(:, j, :) = swHeatingRateClearSky(:, 1:nzm)
                                end if

                                !construct 3-D radiation stream arrays from horizontal slices
                                if(doShipTrackConditionals) then
                                  swUp3D(:, j, :) = swUp(:, 1:nzm)
                                  swDown3D(:, j, :) = swDown(:,1:nzm)
                                  lwUp3D(:, j, :) = lwUp(:, 1:nzm)
                                  lwDown3D(:, j, :) = lwDown(:,1:nzm)
                                end if
		...
		swnsxy(:, :) = NetswDownSurface(:, :)
                ...
                lwdsxy(:, :) = lwDownSurface(:, :) ! Net LW upwards
                swdsxy(:, :) = swDownSurface(:, :) ! Net SW downwards
