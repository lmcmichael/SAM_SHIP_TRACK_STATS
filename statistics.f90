...
use vars
use grid, only: doShipTrackConditionals, day
...
! UW ADDITIONS
 ...
 real, dimension(nzm) :: swu0, swd0, lwu0, lwd0, swut, swdt, lwut, lwdt !rad terms
...
!bloss:  Major modification of conditional statistics to allow 
	    ...
	    tcb(:) = 0.
            ...
            if(doShipTrackConditionals) then
                ...
                !compute inversion tendency
                !zb_new (subdomain) - zb_old (full conditional average)
                ...
                !compute entrain. rate from mass budget (m/s)
                wem(:) = invt(:) - wla(:) - wlo(:)

                call hbuf_put('WEM'//TRIM(condavgname(ncond)),wem,1.)
